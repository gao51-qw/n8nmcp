const docs = {
  ...
  index: {
    title: "Japanese edition: n8n-mcp documentation",
    description:
      "Japanese edition: Documentation for connecting n8n workflows to AI clients through a hosted MCP gateway, including setup, tools, quotas and security.",
    h1: "Japanese edition: n8n-mcp documentation",
    lead: "Japanese edition: n8n-mcp gives MCP-compatible AI clients a controlled gateway to your n8n workflows. These docs cover the hosted endpoint, platform API keys, encrypted n8n instances, workflow tools, quotas and security boundaries.",
    ...
    cards: [
      {
        ...
        title: "Japanese edition: Getting started",
        desc: "Japanese edition: Set up n8n-mcp by creating a platform API key, connecting an n8n instance and configuring an MCP client with the hosted endpoint.",
      },
      {
        to: "/docs/concepts",
        title: "Japanese edition: Concepts",
        desc: "Japanese edition: Core concepts for the hosted n8n MCP gateway: endpoint, platform API keys, encrypted n8n instances, tools and request routing.",
      },
      {
        to: "/docs/clients",
        title: "Japanese edition: Connect a client",
        desc: "Japanese edition: Configure Claude, ChatGPT custom connectors, Cursor, Windsurf, VS Code, Zed and other MCP clients with the n8n-mcp endpoint.",
      },
      {
        to: "/docs/api-keys",
        title: "Japanese edition: Platform API keys",
        desc: "Japanese edition: How to create, label, rotate, revoke and protect nmcp_ platform API keys used by MCP clients.",
      },
      {
        to: "/docs/n8n-instances",
        title: "Japanese edition: n8n instances",
        desc: "Japanese edition: Connect n8n instances to the MCP gateway with encrypted API keys, SSRF checks and safe connection testing.",
      },
      {
        to: "/docs/tools",
        title: "Japanese edition: MCP tools reference",
        desc: "Japanese edition: Reference for MCP tools that list, inspect, create, update, delete, validate, activate and execute n8n workflows.",
      },
      {
        to: "/docs/quotas",
        title: "Japanese edition: Quotas and billing",
        desc: "Japanese edition: How n8n-mcp counts MCP requests, enforces short-window rate limits and applies daily or plan-based quotas.",
      },
      {
        to: "/docs/security",
        title: "Japanese edition: Security model",
        desc: "Japanese edition: Security model for n8n-mcp, including credential encryption, SSRF protection, API key authentication, quotas and tenant boundaries.",
      },
    ],
  },
  gettingStarted: {
    title: "Japanese edition: Getting started with n8n-mcp",
    description:
      "Japanese edition: Set up n8n-mcp by creating a platform API key, connecting an n8n instance and configuring an MCP client with the hosted endpoint.",
    h1: "Japanese edition: Getting started",
    body: '<p><strong>Japanese edition.</strong></p>\n<p>This guide connects one MCP-compatible client to one n8n instance through n8n-mcp. The client talks to the hosted endpoint, while n8n credentials stay encrypted inside the gateway.</p>\n<h2>1. Create a platform API key</h2>\n<p>Open the dashboard, create a platform key and copy the <code>nmcp_...</code> token immediately. The full token is shown once; after that only its hash and display metadata are stored.</p>\n<h2>2. Connect an n8n instance</h2>\n<p>Add your n8n base URL and n8n API key in the dashboard. The API key is encrypted at rest and is only decrypted server-side when the gateway calls n8n.</p>\n<h2>3. Configure your MCP client</h2>\n<pre>{\n  "mcpServers": {\n    "n8n-mcp": {\n      "url": "https://mcp.n8nworkflow.com/mcp",\n      "headers": { "Authorization": "Bearer nmcp_..." }\n    }\n  }\n}</pre>\n<h2>4. Test a read-only tool first</h2>\n<p>Restart the client and ask it to list your n8n workflows. A successful setup should call <code>list_workflows</code> and return workflow names, IDs and active status.</p>',
  },
  concepts: {
    title: "Japanese edition: n8n-mcp concepts",
    description:
      "Japanese edition: Core concepts for the hosted n8n MCP gateway: endpoint, platform API keys, encrypted n8n instances, tools and request routing.",
    h1: "Japanese edition: Concepts",
  ...
  clients: {
    title: "Japanese edition: Connect MCP clients to n8n-mcp",
    description:
      "Japanese edition: Configure Claude, ChatGPT custom connectors, Cursor, Windsurf, VS Code, Zed and other MCP clients with the n8n-mcp endpoint.",
    h1: "Japanese edition: Connect a client",
  ...
  apiKeys: {
    title: "Japanese edition: Platform API keys for n8n-mcp",
    description:
      "Japanese edition: How to create, label, rotate, revoke and protect nmcp_ platform API keys used by MCP clients.",
    h1: "Japanese edition: Platform API keys",
  ...
  n8nInstances: {
    title: "Japanese edition: n8n instances in n8n-mcp",
    description:
      "Japanese edition: Connect n8n instances to the MCP gateway with encrypted API keys, SSRF checks and safe connection testing.",
    h1: "Japanese edition: n8n instances",
  ...
  tools: {
    title: "Japanese edition: n8n-mcp tools reference",
    description:
      "Japanese edition: Reference for MCP tools that list, inspect, create, update, delete, validate, activate and execute n8n workflows.",
    h1: "Japanese edition: MCP tools reference",
  ...
  quotas: {
    title: "Japanese edition: n8n-mcp quotas and billing",
    description:
      "Japanese edition: How n8n-mcp counts MCP requests, enforces short-window rate limits and applies daily or plan-based quotas.",
    h1: "Japanese edition: Quotas and billing",
  ...
  security: {
    title: "Japanese edition: n8n-mcp security model",
    description:
      "Japanese edition: Security model for n8n-mcp, including credential encryption, SSRF protection, API key authentication, quotas and tenant boundaries.",
    h1: "Japanese edition: Security model",
